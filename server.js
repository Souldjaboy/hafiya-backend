...
const allowedOrigins = [
  ...
  "https://hafiya.trianglewmspro.com",
  "https://www.hafiya.trianglewmspro.com",
  ...
  "http://localhost:3030",
  "http://127.0.0.1:3030",
  "https://afia.hafiya.trianglewmspro.com",
  "https://malilink.hafiya.trianglewmspro.com",
  "http://localhost:3000"
].filter(Boolean);
...
function getTenantFromRequest(req) {
  ...
  if (host.includes("malilinkglobal.com") || host.includes("malilink.hafiya.trianglewmspro.com")) {
    return "malilink";
  }
  if (host.includes("hafiyalab.com") || host.includes("afia.hafiya.trianglewmspro.com")) {
    return "hafiya";
  }
...
function productQrUrl(req, product) {
  ...
  const baseUrl =
    ...
    process.env.PUBLIC_BASE_URL ||
    `${host?.includes("hafiya.trianglewmspro.com") ? "https" : forwardedProto}://${host}`;
...
function supportWhatsAppUrl() {
  ...
  if (!number) return "";
  const text = encodeURIComponent("Bonjour HAFIYA Laboratoire, j'ai besoin d'aide");
  return `https://wa.me/${number}?text=${text}`;
}

function publicAppUrl() {
  return String(process.env.APP_URL || process.env.PUBLIC_BASE_URL || process.env.FRONTEND_URL || "https://hafiya.trianglewmspro.com").replace(/\/$/, "");
}
...
async function sendPasswordResetMessage({ targetType, targetValue, code, resetUrl }) {
  if (targetType === "email") {
    ...
    await transporter.sendMail({
      ...
      to: targetValue,
      subject: "Réinitialisation mot de passe HAFIYA Laboratoire",
      text: `Votre code de réinitialisation HAFIYA Laboratoire est : ${code}. Il expire dans 15 minutes.\n\nLien sécurisé : ${resetUrl}`,
      html: `
        <div style="font-family:Arial,sans-serif;color:#111">
...
app.get("/", (req, res) => {
  res.send("HAFIYA Laboratoire Backend sécurisé OK");
});
...
app.get("/company-settings/current", authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      ...
      company_name: settings.company_name || company.name || "HAFIYA Laboratoire",
        ...
        try {
          ...
          await transporter.sendMail({
            ...
            html: `
              ...
              <p>Votre mot de passe temporaire HAFIYA Laboratoire est :</p>
...
app.post("/pos/send-receipt-email", authenticateToken, async (req, res) => {
  try {
    ...
    const appName = companySettings?.company_name || receiptData.company_name || "HAFIYA Laboratoire";
    ...
    <main class="page">
      <section class="header">
        <div>
          ...
          <h2>${escapeHtml(companySettings.name || companySettings.company_name || "HAFIYA Laboratoire")}</h2>
...
app.post("/reports/email", authenticateToken, async (req, res) => {
  try {
    ...
    const { recipient_email = "", subject = "Rapport HAFIYA Laboratoire", html = "", message = "" } = req.body || {};
    if (!recipient_email || !String(recipient_email).includes("@")) {
      return res.status(400).json({ error: "Email destinataire invalide." });
    }
    if (!process.env.SMTP_HOST || !process.env.SMTP_USER || !process.env.SMTP_PASS) {
    ...
    const finalHtml = html || `<p>${escapeHtml(message || "Rapport HAFIYA Laboratoire")}</p>`;
    const info = await transporter.sendMail({
      from: process.env.SMTP_FROM || process.env.SMTP_USER,
      to: recipient_email,
      subject,
      text: message || "Rapport HAFIYA Laboratoire",
      html: finalHtml,
...
async function recordBusinessPayment(client, {
  ...
  await client.query(
    ...
    [
      ...
      amountValue,
      description,
      user?.email || user?.fullname || "HAFIYA Laboratoire",
      companyId,
      sourceType,
      sourceId
    ]
  ...
  try {
    ...
    const meetingResult = await pool.query(
      `INSERT INTO meetings
       ...
       RETURNING *`,
      [
        title || "Réunion HAFIYA Laboratoire",
        roomName,
        meetingUrl,
    ...
    if (item.tool === "get_module_knowledge") {
      ...
      if (modules.length > 0) {
        ...
        modules.slice(0, 3).forEach((module) => {
          lines.push(
            `  • ${module.module_name} : ${module.role_explanation || module.description || "Module HAFIYA Laboratoire."}`
          );
        });
      }
    ...
    const aiResponse = await fetch(
      "https://openrouter.ai/api/v1/chat/completions",
      {
        ...
        headers: {
          ...
          "HTTP-Referer": "https://hafiya.trianglewmspro.com",
  ...
  try {
    ...
    const response = await axios.post(
      "https://api-checkout.cinetpay.com/v2/payment",
      {
        ...
        description: "Abonnement HAFIYA Laboratoire",
...
try {
  require("./hafiya-extra-routes")(app, pool, authenticateToken);
  console.log("✅ Routes HAFIYA extra chargées");
} catch (e) {
  console.error("❌ Erreur routes HAFIYA extra:", e.message);
}


try {
  require("./communication-hub-routes")(app, pool, authenticateToken);
  console.log("✅ Communication Hub chargé");
} catch (e) {
  console.error("❌ Communication Hub erreur:", e.message);
}


try {
  require("./communication-advanced-routes")(app, pool, authenticateToken);
  console.log("✅ Communication Advanced chargé");
} catch (e) {
